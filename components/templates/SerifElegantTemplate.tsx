...
const SerifElegantTemplate: React.FC<TemplateProps> = ({ data }) => {
  return (
    <div className="bg-white w-[21cm] min-h-[29.7cm] p-[1.5cm] text-gray-900 shadow-lg font-serif">
      {/* Header */}
      <header className="mb-8">
        <h1 className="text-5xl font-bold text-[#654321] mb-2 font-serif">
        ...
        <div className="border-t border-b border-[#8B7355] py-2 flex justify-around text-sm text-[#5D4037]">
      ...
      {data.experience.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xl font-bold text-[#654321] border-b border-[#654321] pb-1 mb-4">
            Experience
          </h2>
          <div className="space-y-6 relative border-l-2 border-[#D2B48C] pl-6 ml-2">
            {data.experience.map((exp) => (
              <div key={exp.id} className="relative">
                <div className="absolute -left-[33px] top-1 w-4 h-4 rounded-full bg-[#D2B48C] border-2 border-white"></div>
                ...
                <div className="text-md text-[#654321] font-semibold mb-2">{exp.employer}</div>
      ...
      {data.education.length > 0 && (
        <section className="mb-8">
          ...
          <div className="space-y-4 relative border-l-2 border-[#D2B48C] pl-6 ml-2">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-2">
             {data.skills.map((skill) => (
                <div key={skill} className="flex items-center gap-2 text-gray-800">
                   <span className="text-[#8B7355]">•</span> {skill}
                </div>
             ))}
          </div>
        </section>
