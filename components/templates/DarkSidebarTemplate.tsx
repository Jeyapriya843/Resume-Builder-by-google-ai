...
const DarkSidebarTemplate: React.FC<TemplateProps> = ({ data }) => {
  return (
    <div className="bg-white w-[21cm] min-h-[29.7cm] flex shadow-lg font-sans">
       {/* Left Sidebar (Dark) */}
       <div className="w-[7.5cm] bg-[#2c3e50] text-white p-8 flex flex-col">
          {/* Photo Placeholder */}
       ...
       <div className="flex-1 p-10 bg-white text-gray-800">
          <header className="mb-12">
             <h1 className="text-5xl font-bold text-[#2c3e50] mb-2">{data.firstName} {data.lastName}</h1>
          ...
          <section>
             <h2 className="text-2xl font-bold text-[#2c3e50] border-b-2 border-[#2c3e50] pb-2 mb-6">
                Experience
             </h2>
             <div className="space-y-8">
                {data.experience.map(exp => (
                   <div key={exp.id} className="relative pl-6 border-l-2 border-gray-200">
                      <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 border-[#2c3e50] bg-white"></div>
                      ...
                      <div className="text-md font-semibold text-[#2c3e50] mb-2">{exp.jobTitle}</div>
