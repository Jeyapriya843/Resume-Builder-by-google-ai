...
const ExperienceStep: React.FC = () => {
  ...
  const handleAiAction = async (id: string, title: string, employer: string, mode: 'generate' | 'enhance', currentDesc?: string) => {
    if (mode === 'generate' && (!title || !employer)) {
      alert("Please enter both Job Title and Employer first to help the AI generate relevant content.");
      return;
    }
    
    if (mode === 'enhance' && (!currentDesc || currentDesc.trim().length < 5)) {
      alert("Please write some bullet points first to use the Enhance feature.");
      return;
    }
